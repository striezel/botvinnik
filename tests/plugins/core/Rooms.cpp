...
TEST_CASE("plugin Rooms")
{
  ...
  SECTION("commands must exist")
  {
    ...
    // Rooms command has to be there.
    REQUIRE_FALSE( std::find(commands.begin(), commands.end(), "rooms") == commands.end() );
    // Leave command would be nice to have, too.
    REQUIRE_FALSE( std::find(commands.begin(), commands.end(), "leave") == commands.end() );
  }
