...
TEST_CASE("plugin Giphy")
{
  using namespace bvn;
  using namespace std::chrono;
  Configuration conf;
  Bot bot(conf);
  Giphy plugin(conf.gifApiKey(), bot.matrix());
  ...
  SECTION("commands must exist")
  {
    ...
    // giphy command has to be there.
    REQUIRE_FALSE( std::find(commands.begin(), commands.end(), "giphy") == commands.end() );
  }
