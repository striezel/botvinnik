...
TEST_CASE("plugin Fortune")
{
  ...
  SECTION("command handlers must return text")
  {
    ...
    for (const auto& cmd : commands)
    {
      ...
      REQUIRE_FALSE( plugin.handleCommand(cmd, cmd, mockUserId, mockRoomId, ts).formatted_body.empty() );
    }
  }
  ...
  SECTION("executable path must not be empty")
  {
    REQUIRE_FALSE( plugin.executableLocation().empty() );
  }
}
